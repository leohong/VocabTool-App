...
* 🎲 **三段特訓模式切換**：支援全域切換 `🎴 閃卡模式`、`🎲 隨機模式（預設）` 與 `🔘 4選1選擇題模式`（適用於「降溫：錯題大會考」與「深度：歷史隨機抽查」，隨機模式下每道測驗題 50% 機率為拼寫或四選一）。
...
### 1. 指揮中心按鈕功能
* **⚡ 發動今日特訓**：開始當天設定的新單字特訓（第一關為閃卡篩選，第二關固定為強制盲測全拼寫）。特訓中會自動安插今日到期的歷史「幽靈單字 👻」進行突襲。
* **🔥 降溫：錯題大會考**：從當前錯題集中隨機抽取最多 50 個字進行特訓：
  - 在 `🎴 閃卡模式` 下：進行「純拼寫盲測」。
  - 在 `🎲 隨機模式` 下（預設）：每字隨機 50% 為「四選一選擇題」或「純拼寫盲測」。
...
### 2. 特訓模式開關說明 (⚡ 特訓模式)
位於儀表板「📢 自動發音」下方，提供全域膠囊三選一切換（適用於「降溫」與「深度」測驗）：
* **`🎴 閃卡`**：
  - 今日特訓：第一關閃卡瀏覽 ➔ 第二關固定為強制盲測全拼寫。
  - 錯題大會考 / 歷史隨機抽查：進行純拼字測驗。
* **`🎲 隨機`（預設）**：
  - 今日特訓：第一關閃卡瀏覽 ➔ 第二關固定為強制盲測全拼寫。
  - 錯題大會考 / 歷史隨機抽查：每字隨機 50% 為四選一 MCQ 或拼寫盲測。
* **`🔘 4選1`**：
  - 今日特訓：第一關閃卡瀏覽 ➔ 第二關固定為強制盲測全拼寫。
