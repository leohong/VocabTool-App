...
* ⚡ **雙階段流水線訓練**：第一關快速閃卡篩選 ➔ 第二關測驗（預設為「🎲 隨機」混合拼寫與四選一 / 可切換純閃卡或純四選一）。
* 🎲 **三段特訓模式切換**：支援全域切換 `🎴 閃卡模式`、`🎲 隨機模式（預設）` 與 `🔘 4選1選擇題模式`，隨機模式下每道題 50% 機率為拼寫或四選一。
...
### 2. 特訓模式開關說明 (⚡ 特訓模式)
位於儀表板「📢 自動發音」下方，提供全域膠囊三選一切換：
* **`🎴 閃卡`**：
  - 今日特訓：第一關閃卡瀏覽 ➔ 第二關強制盲測全拼寫。
  - 錯題大會考 / 歷史隨機抽查：直接進行純拼字測驗。
* **`🎲 隨機`（預設）**：
  - 今日特訓：第一關閃卡瀏覽 ➔ 第二關每字隨機 50% 為四選一 MCQ 或拼寫盲測。
  - 錯題大會考 / 歷史隨機抽查：每字隨機 50% 為四選一或拼寫（兩種題型混合出現）。
* **`🔘 4選1`**：
