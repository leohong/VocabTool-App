...
### 1. 指揮中心按鈕功能
* **⚡ 發動今日特訓**：開始當天設定的新單字特訓（第一關為閃卡篩選，第二關為強制拼寫）。特訓中會自動安插今日到期的歷史「幽靈單字 👻」進行突襲。
...
* **`🎴 閃卡`（預設）**：今日特訓第一關為閃卡瀏覽，錯題大會考與歷史抽查為純拼寫測驗。
* **`🔘 4選1`**：今日特訓第一關保持閃卡瀏覽，錯題大會考與歷史抽查轉為 4 選 1 中文選擇題測驗（4選1模式下例句自動遮蔽中文翻譯）。
...
| | 第一關：4選1測驗 | **A / B / C / D 選擇題按鈕** | 點選正確中文答案推進 / 點選錯誤即記為錯題 |
