...
位於主儀表板「📢 自動發音」下方，提供 `[ 🎴 閃卡模式 | 🎲 隨機模式 | 🔘 4選1模式 ]` 全域膠囊切換鈕（切換結果套用於「🔥 降溫：錯題大會考」與「🏛️ 深度：歷史隨機抽查」；「⚡ 今日特訓」第二階段固定為強制盲測全拼寫）：

*   **`🎴 閃卡模式`**：
    *   **⚡ 今日特訓**：第一關為閃卡瀏覽（認識／不熟）➔ 第二關固定為強制盲測全拼寫。
    *   **🔥 錯題大會考**：進行純拼寫盲測（100% 原始拼字測驗）。
    *   **🏛️ 歷史隨機抽查**：進行純拼寫盲測（100% 原始拼字抽驗）。
