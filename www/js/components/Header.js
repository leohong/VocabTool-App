...
window.Header = ({
  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-stretch md:items-center mb-6 bg-slate-800 p-3 sm:p-4 rounded-xl shadow-md border border-slate-700 gap-2.5 sm:gap-4">
      {/* 標題與說明書按鈕 */}
      <div className="flex justify-between items-center w-full md:w-auto gap-2">
        ...
        <a
          ...
          className="px-2 py-1 text-[10px] font-medium text-indigo-300 bg-indigo-950/50 hover:bg-indigo-900/60 border border-indigo-500/30 rounded-md transition-colors flex items-center gap-1 shrink-0"
        >
          📖 說明書
        </a>
      </div>
      ...
      {/* 狀態、打卡與進度選單 */}
      <div className="flex items-center justify-between sm:justify-end w-full md:w-auto gap-1.5 sm:gap-2">
        {/* 左側：狀態燈號與打卡天數 */}
