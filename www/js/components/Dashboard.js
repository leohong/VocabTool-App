  ...
  return (
    <div className="w-full flex flex-col gap-5 animate-[fadeIn_0.4s_ease-in-out]">
      ...
      {vocabList && vocabList.length === 0 ? (
        <div className="w-full bg-slate-800 border-2 border-dashed border-indigo-500/50 p-6 rounded-2xl text-center space-y-3 shadow-xl">
          <div className="text-3xl">🗂️</div>
          <h2 className="text-lg font-bold text-white">目前字庫【{dbName}】尚無單字</h2>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            您可以手動新增自訂單字，或使用下方的「⬆️ 字」匯入 JSON/TXT 單字檔。
          </p>
          <div className="flex justify-center gap-2 pt-2">
            <button
              onClick={() => { setShowDictModal(true); resetDictState(); }}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold shadow-md transition-colors flex items-center gap-1"
            >
              📖 管理/新增自訂單字
            </button>
            <button
              onClick={() => setShowImportOptionsModal(true)}
              className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-xl text-xs font-bold transition-colors flex items-center gap-1"
            >
              ⬆️ 匯入單字檔 (JSON/TXT)
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex justify-between items-center gap-1">
            ...
            <input
              ...
              max="500"
              value={wordsPerDay}
              onChange={(e) => setWordsPerDay(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
              onBlur={(e) => setWordsPerDay(Math.max(1, parseInt(e.target.value, 10) || 1))}
          ...
          <div className="flex justify-between items-center gap-1 border-l border-slate-700 pl-2">
            ...
            <input
              ...
              max="100"
              value={ghostsPerDay}
              onChange={(e) => setGhostsPerDay(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
              onBlur={(e) => setGhostsPerDay(Math.max(0, parseInt(e.target.value, 10) || 0))}
