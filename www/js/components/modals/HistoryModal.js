      ...
      <div className="bg-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[85vh] border border-slate-700">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-2">
          {historyTotal === 0 ? (
            ...
            Object.values(historicalMistakes || {})
              .sort((a, b) => (b?.totalFails || 0) - (a?.totalFails || 0))
              .map((item) => {
                const wordData = resolveWord(item);
                const archivedTime = item?.archivedDate || Date.now();
                const intervalDays = item?.interval || 7;
                const nextDate = new Date(archivedTime + intervalDays * 24 * 60 * 60 * 1000);
                return (
                  <div key={wordData.en || Math.random()} className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 sm:p-4 bg-slate-900/50 rounded-xl border border-slate-700/60 gap-3 sm:gap-4">
                    <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-baseline gap-1.5 sm:gap-2">
                          <span className="font-bold font-mono text-base sm:text-lg text-amber-100 break-all">{wordData.en}</span>
                    ...
                    <div className="flex flex-col gap-1 w-full sm:w-48 shrink-0 text-left sm:text-right pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                      <span className="text-xs font-mono text-slate-500">魔王等級 (失誤): {item?.totalFails || 0} 次</span>
                      {item?.immune ? (
