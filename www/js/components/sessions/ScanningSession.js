...
window.ScanningSession = ({
  ...
  // 當 currentWord 變更時：進行清理、自動發音判斷與選項產生
  React.useEffect(() => {
    ...
    // 觸發全域自動發音 (isManual = false 遵循全域 speechEnabled 開關)
    speak(currentWord.en, false);
  ...
  }, [currentWord, isMcqMode, vocabList]);
  ...
  // 4選1 模式鍵盤快捷鍵支援 (僅使用 1, 2, 3, 4)
  React.useEffect(() => {
    if (!isMcqMode || isAnswered || mcqChoices.length < 4) return;

    const handleMcqKeyDown = (e) => {
      ...
      const targetIdx = keyMap[e.key] ?? keyMap[e.code];
      if (targetIdx !== undefined && mcqChoices[targetIdx]) {
        e.preventDefault();
        handleMcqSelect(targetIdx, mcqChoices[targetIdx]);
      }
    };

    window.addEventListener('keydown', handleMcqKeyDown);
    return () => {
      window.removeEventListener('keydown', handleMcqKeyDown);
    };
  }, [isMcqMode, isAnswered, mcqChoices]);
